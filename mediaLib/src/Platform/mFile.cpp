...
mFUNCTION(mFile_CreateDirectory, const mString &folderPath)
{
  ...
  const int errorCode = SHCreateDirectoryExW(nullptr, wDirectoryName, nullptr);
...
mFUNCTION(mFile_GetCurrentModuleFilePath, OUT mString *pModuleDirectory, IN OPTIONAL const void *pFunctionInModule /* = nullptr */)
{
  ...
  mERROR_IF(pModuleDirectory == nullptr, mR_ArgumentNull);

  HMODULE moduleHandle = nullptr;

  if (pFunctionInModule == nullptr)
    pFunctionInModule = &mFile_GetCurrentModuleFilePath;

  if (GetModuleHandleEx(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT, reinterpret_cast<LPWSTR>(const_cast<void *>(pFunctionInModule)), &moduleHandle) == 0)
    moduleHandle = nullptr;

  wchar_t filePath[MAX_PATH];

  const DWORD result = GetModuleFileNameW(moduleHandle, filePath, (DWORD)mARRAYSIZE(filePath));
  ...
  if (result != 0)
  {
    ...
    {
    case ERROR_SUCCESS:
      mERROR_CHECK(mString_Create(pModuleDirectory, filePath, pModuleDirectory->pAllocator));
      mRETURN_SUCCESS();
      break;
...
mFUNCTION(mFile_GetDirectoryContents, const mString &directoryPath, const mString &searchTerm, OUT mPtr<mQueue<mFileInfo>> *pFiles, IN mAllocator *pAllocator)
{
  ...
  const HANDLE handle = FindFirstFileExW(absolutePath, FindExInfoBasic, &fileData, FindExSearchNameMatch, nullptr, FIND_FIRST_EX_LARGE_FETCH);
...
mFUNCTION(mFile_GetInfo, const mString &filename, OUT mFileInfo *pFileInfo)
{
  ...
  HANDLE fileHandle = CreateFileW(wfilename, STANDARD_RIGHTS_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
...
mFUNCTION(mFile_CreateShortcut, const mString &path, const mString &targetDestination, const mString &arguments, const mString &workingDirectory, const mString &description, const mString &iconLocation)
{
  ...
  HRESULT hr = CoCreateInstance(CLSID_ShellLink, nullptr, CLSCTX_ALL, IID_IShellLink, (void **)&pShellLink);
...
HRESULT mFile_CreateAndInitializeFileOperation_Internal(REFIID riid, void **ppFileOperation)
{
  *ppFileOperation = nullptr;
  ...
  HRESULT hr = CoCreateInstance(__uuidof(FileOperation), nullptr, CLSCTX_ALL, IID_PPV_ARGS(&pfo));
...
mFUNCTION(mRegistry_WriteKey, const mString &keyUrl, const mString &value, OUT OPTIONAL bool *pNewlyCreated /* = nullptr */)
{
  ...
  LSTATUS result = RegCreateKeyExW(parentKey, wPathOrValue, 0, nullptr, 0, KEY_ALL_ACCESS, nullptr, &key, &disposition);
...
mFUNCTION(mRegistry_WriteKey, const mString &keyUrl, const uint32_t value, OUT OPTIONAL bool *pNewlyCreated /* = nullptr */)
{
  ...
  result = RegSetValueExW(key, wValueName, 0, REG_DWORD, reinterpret_cast<const BYTE *>(&value), sizeof(value));
  mERROR_IF(result != ERROR_SUCCESS, mR_InternalError);
...
mFUNCTION(mRegistry_WriteKey, const mString &keyUrl, const uint64_t value, OUT OPTIONAL bool *pNewlyCreated /* = nullptr */)
{
  ...
  result = RegSetValueExW(key, wValueName, 0, REG_QWORD, reinterpret_cast<const BYTE *>(&value), sizeof(value));
  mERROR_IF(result != ERROR_SUCCESS, mR_InternalError);
...
mFUNCTION(mRegistry_ReadKey, const mString &keyUrl, OUT mString *pValue)
{
  ...
  mDEFER_IF(key != nullptr, RegCloseKey(key));
  mERROR_IF(result == ERROR_ACCESS_DENIED, mR_InsufficientPrivileges);
  mERROR_IF(result != ERROR_SUCCESS, mR_ResourceNotFound);

  DWORD type = 0;
  DWORD bytes = 0;

  result = RegQueryValueExW(key, wValueName, nullptr, &type, nullptr, &bytes);
  ...
  if (bytes > wPathOrValueCount * sizeof(wchar_t))
  {
    wPathOrValueCount = (bytes + 1) / sizeof(wchar_t);
    mERROR_CHECK(mAllocator_Reallocate(pAllocator, &wPathOrValue, wPathOrValueCount));
  }

  result = RegQueryValueExW(key, wValueName, nullptr, nullptr, reinterpret_cast<BYTE *>(wPathOrValue), &bytes);
  ...
  mERROR_CHECK(mString_Create(pValue, wPathOrValue, wPathOrValueCount, pValue->pAllocator));
...
mFUNCTION(mRegistry_ReadKey, const mString &keyUrl, OUT uint32_t *pValue)
{
  ...
  mERROR_IF(type != REG_DWORD && type != REG_DWORD_BIG_ENDIAN, mR_ResourceStateInvalid);
  mERROR_IF(bytes != sizeof(uint32_t), mR_InternalError);

  result = RegQueryValueExW(key, wValueName, nullptr, nullptr, reinterpret_cast<BYTE *>(pValue), &bytes);
  ...
  if (type == REG_DWORD_BIG_ENDIAN)
    *pValue = ((*pValue >> 24) & 0xFF) | ((*pValue >> 8) & 0xFF00) | ((*pValue << 8) & 0xFF0000) | ((*pValue << 24) & 0xFF000000);
...
mFUNCTION(mRegistry_ReadKey, const mString &keyUrl, OUT uint64_t *pValue)
{
  ...
  mERROR_IF(type != REG_QWORD, mR_ResourceStateInvalid);
  mERROR_IF(bytes != sizeof(uint32_t), mR_InternalError);
...
mFUNCTION(mRegistry_FindKey_Internal, const mString &keyUrl, OUT HKEY *pParentKey, IN mAllocator *pAllocator, OUT wchar_t **pPathOrValue, OUT size_t *pPathOrValueCount, OUT wchar_t **pValueName, OUT size_t *pValueNameCount)
{
  ...
  if (keyUrl.StartsWith(classesRoot))
  {
    *pParentKey = HKEY_CLASSES_ROOT;

    mERROR_CHECK(mString_Substring(keyUrl, &subAddress, mARRAYSIZE(classesRoot) - 1));
  }
  else if (keyUrl.StartsWith(currentConfig))
  {
    *pParentKey = HKEY_CURRENT_CONFIG;

    mERROR_CHECK(mString_Substring(keyUrl, &subAddress, mARRAYSIZE(currentConfig) - 1));
  }
  else if (keyUrl.StartsWith(currentUser))
  {
    *pParentKey = HKEY_CURRENT_USER;

    mERROR_CHECK(mString_Substring(keyUrl, &subAddress, mARRAYSIZE(currentUser) - 1));
  }
  else if (keyUrl.StartsWith(localMachine))
  {
    *pParentKey = HKEY_LOCAL_MACHINE;

    mERROR_CHECK(mString_Substring(keyUrl, &subAddress, mARRAYSIZE(localMachine) - 1));
  }
  else if (keyUrl.StartsWith(users))
  {
    *pParentKey = HKEY_USERS;
  ...
  mERROR_CHECK(mString_GetRequiredWideStringCount(pathString, pPathOrValueCount));
  mERROR_CHECK(mAllocator_AllocateZero(pAllocator, pPathOrValue, *pPathOrValueCount));

  mERROR_CHECK(mString_ToWideString(pathString, *pPathOrValue, *pPathOrValueCount));

  mERROR_CHECK(mString_GetRequiredWideStringCount(valueName, pValueNameCount));
  mERROR_CHECK(mAllocator_AllocateZero(pAllocator, pValueName, *pValueNameCount));

  mERROR_CHECK(mString_ToWideString(valueName, *pValueName, *pValueNameCount));
