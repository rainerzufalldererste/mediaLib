...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if ((hovered || g.HoveredId == id || g.ActiveId == id) && (format != nullptr && format[0] != '\0'))
    {
      ...
      ImVec2 textCenter = grab_bb.GetCenter() - ImVec2(0, valueSize.y + style.ItemInnerSpacing.y * 2.f * g.IO.FontGlobalScale);
      
      window->DrawList->AddTriangleFilled(textCenter + ImVec2(-style.ItemInnerSpacing.y * g.IO.FontGlobalScale, style.ItemInnerSpacing.y * g.IO.FontGlobalScale + valueSize.y * .5f), textCenter + ImVec2(style.ItemInnerSpacing.y * g.IO.FontGlobalScale, style.ItemInnerSpacing.y * g.IO.FontGlobalScale + valueSize.y * .5f), textCenter + ImVec2(0, style.ItemInnerSpacing.y * 2.f * g.IO.FontGlobalScale + valueSize.y * .5f), GetColorU32(ImGuiCol_FrameBg));

      const ImVec2 halfValueSize = valueSize * .5f;
      const ImVec2 halfValueSizeBg = halfValueSize + style.ItemInnerSpacing * g.IO.FontGlobalScale;
